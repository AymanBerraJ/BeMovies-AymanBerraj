...
document.getElementById('button_search').addEventListener('click', () => {
  ...
  if (valueInput) {
      ...
      // Appeler la fonction de recherche de films avec la nouvelle recherche
...
async function movieGenre() {
  ...
  try {
    const response = await fetch(
      `https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=false&language=en-US&page=1&sort_by=popularity.desc&with_genres=`, // pas oublier de mettre ${} apres genre
